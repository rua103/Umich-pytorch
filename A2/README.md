...
## 3. 工具和实验支持（`eecs598/a2_helpers.py`）

### 3.1 数据与种子
- `reset_seed(num)`：固定随机种子，保证复现性
- `get_toy_data(...)`：小规模测试集（用于快速验证实现）

### 3.2 可视化函数
- `plot_stats` / `plot_acc_curves`：loss/accuracy 曲线监控
- `visualize_grid` + `show_net_weights`：权重可视化调试
...
1. ✅ 理解 hinge loss 公式和含义
2. ✅ 推导 naive 双循环 → 梯度
3. ✅ 理解 mask 的构造与作用
4. ✅ 向量化实现（$X^T @ \text{mask}$）
5. ✅ 代码验证 naive vs vectorized
6. ✅ 性能对比（timeit）

### 4.2 调试策略

- 先小数据（toy_data）验证
- 对比 naive 和 vectorized 的 loss/dW
- 逐步放大数据规模
- 画 loss 曲线检查是否收敛
