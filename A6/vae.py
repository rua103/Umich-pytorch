...
def discriminator_loss(logits_real, logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    true_labels = torch.ones_like(logits_real,device=logits_real.device,dtype=logits_real.dtype)
    fake_labels = torch.zeros_like(logits_fake,device=logits_fake.device,dtype=logits_fake.dtype)
...
def get_optimizer(model):
    ...
    learning_rate = 1e-3
    betas = (0.5,0.999)
    optimizer = torch.optim.Adam(params, lr=learning_rate, betas=betas)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return optimizer
