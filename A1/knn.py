...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flattened = x_train.reshape(num_train,-1)
    x_test_flattened = x_test.reshape(num_test,-1)
    for i in range(num_train):
      for j in range(num_test):
        dists[i,j] = torch.square(x_train_flattened[i] - x_test_flattened[j]).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flattened = x_test.reshape(num_test,-1)
    for i in range(num_train):
      dists[i, :] = torch.sum((x_test_flattened - x_train_flattened[i])**2,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_sq = torch.sum(x_test_flat**2, dim=1).view(1, -1)
    two_xy = 2 * torch.matmul(x_train_flat, x_test_flat.T)

    dists = x_train_sq + x_test_sq - two_xy
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    topk_values,topk_indices= torch.topk(dists,k,dim=0,largest=False)#topk_indices.shape(k,num_test)
    topk_indices = topk_indices.t()
    y_train_topk = y_train[topk_indices]

    labels_count = torch.zeros(num_test,y_train.max().item()+1, dtype=torch.int64)
    labels_count.scatter_add_(1, y_train_topk, torch.ones_like(y_train_topk))
    y_pred = torch.argmax(labels_count, dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for i in range(num_folds):
        ...
        for k in k_choices:
            acc = knn.check_accuracy(x_val, y_val, k=k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    ks = sorted(k_to_accuracies.keys())
    accuracies_matrix = torch.tensor([k_to_accuracies[k] for k in ks])
    mean_accuracies = torch.mean(accuracies_matrix, dim=1)
    best_idx = torch.argmax(mean_accuracies)
    best_k = ks[best_idx]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
